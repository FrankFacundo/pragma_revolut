...
flowchart TD
    ...
    forward --> json_out["JSON output"]

    dataset --> parquet["read users/events/profiles parquet"]
    record_node --> enc_record["PragmaTokenizer.encode_record"]
    ...
    bb --> event_encoder_node["event_encoder<br/>TransformerEncoder"]
    ...
    event_encoder_node --> block_e["TransformerBlock x event_layers"]
